...
public static class Ed25519Extensions
{
    ...
    private static bool IsOnCurve(BigInteger x, BigInteger y)
    {
        ...
        var yy = y * y;
        var dxxyy = D * yy * xx;
        return (yy - xx - dxxyy - BigInteger.One).Mod(Q).Equals(BigInteger.Zero);
    }
    ...
    {
        var y = new BigInteger(key) & Un;
        var x = RecoverX(y);

        // Org.BouncyCastle.Math.EC.Rfc8032.Ed25519.ValidatePublicKeyPartial(key); // ? dont know how check / compare results
